        ...
        <div className="flex justify-between items-center">
          ...
          <Link href="/" className="relative z-10">
            <div className="flex items-center">
              ...
              <div className="font-bold text-3xl font-jockey-one">
          ...
          <nav className="hidden md:flex items-center space-x-8">
            {navLinks.map((link) => (
              <Link 
                ...
                } hover:text-[#f36d21] transition-colors font-medium font-jockey-one text-lg`}
              >
                {link.name}
              </Link>
            ))}
            <Link 
              ...
              } px-6 py-2 rounded-full hover:shadow-lg transition-all font-medium font-jockey-one`}
            >
              Register for UTPT
            </Link>
          </nav>
                ...
                <nav className="flex-1 flex flex-col items-center justify-center space-y-6">
                  {navLinks.map((link, index) => (
                    ...
                    >
                      <Link 
                        href={link.href}
                        className="inline-block text-gray-800 hover:text-[#f36d21] transition-colors text-lg font-itim py-2"
                        onClick={(e) => handleNavClick(link.href, e)}
                      >
                        {link.name}
                      </Link>
                  ...
                  >
                    <Link 
                      href="/register"
                      className="inline-block bg-[#f36d21] text-white px-6 py-2 rounded-full hover:bg-[#e15d11] transition-all text-lg font-jockey-one"
                      onClick={() => setMobileMenuOpen(false)}
                    >
                      Register for UTPT
                    </Link>
